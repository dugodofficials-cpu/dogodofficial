...
exports.basicSecurity = exports.strictSecurity = exports.securityMiddleware = void 0;
...
const securityMiddleware = (config = {}) => {
    const settings = Object.assign(Object.assign({}, defaultConfig), config);
    return (req, res, next) => {
        try {
                ...
                for (const pattern of suspiciousPatterns) {
                    if (pattern.test(fullCheck)) {
                        if (settings.logSuspiciousActivity) {
                            logger_1.logger.warn(`SECURITY ALERT: Blocked suspicious request from ${req.ip}`, {
                                method,
                                url,
                                query,
                                userAgent,
                        ...
                        if (settings.strictMode) {
                            ...
                            throw new HttpException_1.HttpException(400, 'Bad Request');
                        }
                    }
                }
            }
            if (settings.blockAirflowExploits) {
                if (url.includes('/api/experimental/') || url.includes('/dag_runs') || url.includes('trigger_dag')) {
                    if (settings.logSuspiciousActivity) {
                        logger_1.logger.warn(`SECURITY ALERT: Blocked Airflow exploitation attempt from ${req.ip}`, {
                            method,
                            url,
                            userAgent,
                            clientIp: req.ip,
                ...
                for (const param of suspiciousParams) {
                    if (query[param]) {
                        ...
                        if (value.includes('../') || value.includes('..\\') || value.includes('/etc/') || value.includes('usr/local/lib')) {
                            if (settings.logSuspiciousActivity) {
                                logger_1.logger.warn(`SECURITY ALERT: Blocked suspicious parameter from ${req.ip}`, {
                                    method,
                                    url,
            ...
            if (url.includes('\0') || JSON.stringify(query).includes('\0')) {
                if (settings.logSuspiciousActivity) {
                    logger_1.logger.warn(`SECURITY ALERT: Blocked null byte injection from ${req.ip}`, {
                        method,
                        url,
                        query,
                        userAgent,
...
exports.strictSecurity = (0, exports.securityMiddleware)({
    blockSuspiciousQueries: true,
    blockPathTraversal: true,
    blockAirflowExploits: true,
    logSuspiciousActivity: true,
...
exports.basicSecurity = (0, exports.securityMiddleware)({
    blockSuspiciousQueries: true,
    blockPathTraversal: true,
    blockAirflowExploits: true,
    logSuspiciousActivity: true,
