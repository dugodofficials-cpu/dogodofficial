...
import { Countdown, CountdownStatus } from '@/modules/countdown/countdown.interface';
const countdownSchema: Schema = new Schema({
  ...
  status: {
    type: String,
    enum: Object.values(CountdownStatus),
    required: true,
    default: CountdownStatus.ACTIVE,
  },
  isActive: {
    type: Boolean,
    required: true,
...
countdownSchema.pre(['findOneAndUpdate', 'updateOne', 'updateMany'], async function (next) {
  ...
  if (update && update.isActive === true) {
    ...
    await countdownModel.updateMany(
      { _id: { $ne: filter._id } },
      { isActive: false }
    );
  }
...
const countdownModel = model<Countdown & Document>('Countdown', countdownSchema);
