...
class ProductService {
  ...
  public async findProductById(productId: string): Promise<Product> {
    ...
    const findProduct: Product = await this.products.findOne({ _id: productId }).populate('albumId');
  ...
  public async findProductBySku(sku: string): Promise<Product> {
    ...
    const findProduct: Product = await this.products.findOne({ sku: sku.toUpperCase() }).populate('albumId');
    if (!findProduct) throw new HttpException(409, "Product doesn't exist");
    return findProduct;
  }
  public async createProduct(productData: CreateProductDto): Promise<Product> {
    if (isEmpty(productData)) throw new HttpException(400, 'productData is empty');
    const findProduct: Product = await this.products.findOne({ sku: productData.sku.toUpperCase() });
  ...
  public async updateProduct(productId: string, productData: UpdateProductDto): Promise<Product> {
    ...
    const updateProductById: Product = await this.products.findByIdAndUpdate(productId, productData, { new: true }).populate('albumId');
  ...
  public async updateStock(productId: string, quantity: number): Promise<Product> {
    ...
    const updatedProduct = await this.products.findByIdAndUpdate(
      productId,
      {
        ...
        $set: {
          status: product.stockQuantity + quantity === 0 ? ProductStatus.OUT_OF_STOCK : ProductStatus.ACTIVE,
        },
      },
      { new: true },
    );
  ...
  public async updateDigitalDeliveryInfo(productId: string, updateData: UpdateDigitalDeliveryInfoDto): Promise<Product> {
    ...
    const updateProductById: Product = await this.products.findByIdAndUpdate(
      productId,
