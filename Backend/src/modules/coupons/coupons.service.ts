...
class CouponService {
  ...
  public async findCouponById(couponId: string): Promise<Coupon> {
    ...
    const findCoupon: Coupon = await this.coupons.findOne({ _id: couponId });
  ...
  public async findCouponByCode(code: string): Promise<Coupon> {
    ...
    const findCoupon: Coupon = await this.coupons.findOne({ code: code.toUpperCase() });
    if (!findCoupon) throw new HttpException(409, "Coupon doesn't exist");
    return findCoupon;
  }
  ...
  public async updateCoupon(couponId: string, couponData: UpdateCouponDto): Promise<Coupon> {
    ...
    const updateCouponById: Coupon = await this.coupons.findByIdAndUpdate(
      couponId,
  ...
  public async deleteCoupon(couponId: string): Promise<Coupon> {
    const deleteCouponById: Coupon = await this.coupons.findByIdAndDelete(couponId);
  ...
  public async validateCoupon(validationData: ValidateCouponDto): Promise<{
    ...
    try {
      ...
      if (validationData.productIds?.length > 0 || validationData.categoryIds?.length > 0) {
