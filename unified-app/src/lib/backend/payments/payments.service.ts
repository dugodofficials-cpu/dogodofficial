import { Order } from '@backend/orders/orders.interface';
...
} from '@backend/payments/payments.dto';
import {
  Payment,
  PaymentRefund,
  PaymentStatus,
  ...
  PaymentAmount,
  PaymentDocument,
} from '@backend/payments/payments.interface';
import { PaymentModel, PaymentRefundModel, PaymentTransactionModel, PaymentTransactionDocument, PaymentRefundDocument } from '@backend/payments/payments.model';
...
export class PaymentService {
  ...
  public async createPayment(paymentData: CreatePaymentDto): Promise<Payment> {
    ...
    const order = await OrderModel.findById(paymentData.order);
    if (!order) {
