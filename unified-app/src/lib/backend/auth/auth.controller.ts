...
class AuthController {
  ...
  public logOut = async (req: RequestWithUser, res: Response, next: NextFunction) => {
    try {
      ...
      const token = req.cookies['Authorization'] || (authHeader ? authHeader.split('Bearer ')[1] : null);
  ...
  public revokeAllOtherSessions = async (req: RequestWithUser, res: Response, next: NextFunction) => {
    try {
      const userData: User = req.user;
      const authHeader = req.header('Authorization');
      const currentToken = req.cookies['Authorization'] || (authHeader ? authHeader.split('Bearer ')[1] : null);
