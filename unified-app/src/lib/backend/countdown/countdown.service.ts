...
import { Countdown, CountdownQueryParams, PaginatedCountdownResponse, CountdownTimeRemaining, CountdownStatus } from '@backend/countdown/countdown.interface';
import countdownModel from '@backend/countdown/countdown.model';
...
class CountdownService {
  ...
  public async updateCountdown(countdownId: string, countdownData: UpdateCountdownDto): Promise<Countdown> {
    ...
    const updateCountdownById = await this.countdowns.findByIdAndUpdate(
      countdownId,
      updateData,
      { new: true }
    );
  ...
  public async activateCountdown(countdownId: string): Promise<Countdown> {
    if (isEmpty(countdownId)) throw new HttpException(400, 'CountdownId is empty');
    await this.countdowns.updateMany({ _id: { $ne: countdownId } }, { isActive: false });
    const activateCountdownById = await this.countdowns.findByIdAndUpdate(
      countdownId,
  ...
  public async deactivateCountdown(countdownId: string): Promise<Countdown> {
    if (isEmpty(countdownId)) throw new HttpException(400, 'CountdownId is empty');
    const deactivateCountdownById = await this.countdowns.findByIdAndUpdate(
      countdownId,
      { isActive: false },
      { new: true }
    );
