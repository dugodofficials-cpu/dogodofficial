...
import { CreateOrderDto, UpdateOrderDto, UpdateOrderStatusDto, UpdateDeliveryStatusDto, GetOrdersQueryDto, BulkDeleteOrdersDto } from '@backend/orders/orders.dto';
import { Order, OrderStatistics, OrderStatus } from '@backend/orders/orders.interface';
...
class OrderController {
  ...
  public getUserOrders = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const limit: number | null = parseInt(req.query.limit as string) || null;
