import { CreateCartDto, UpdateCartDto, AddItemDto, UpdateItemDto, ApplyDiscountDto, UpdateShippingMethodDto } from '@backend/cart/cart.dto';
...
import cartModel from '@backend/cart/cart.model';
import productModel from '@backend/products/products.model';
import orderModel from '@backend/orders/orders.model';
import { Product, ProductType } from '@backend/products/products.interface';
...
class CartService {
  ...
  public async findUserActiveCart(userId: string): Promise<Cart | null> {
    const findCart = await this.carts
      .findOne({
        user: userId,
  ...
  public async createCart(cartData: CreateCartDto): Promise<Cart> {
    ...
    if (!cartData.user) throw new HttpException(400, 'user is required');
  ...
  public async updateItem(cartId: string, productId: string, itemData: UpdateItemDto): Promise<Cart> {
    ...
    if (product.type === ProductType.PHYSICAL && (product.stockQuantity ?? 0) < itemData.quantity) {
  ...
  private async validateAndProcessItems(items: any[]): Promise<void> {
    await Promise.all(
      items.map(async item => {
        ...
        if (product.type === ProductType.PHYSICAL && (product.stockQuantity ?? 0) < item.quantity) {
          throw new HttpException(400, `Insufficient stock for product ${product.sku}`);
        }
