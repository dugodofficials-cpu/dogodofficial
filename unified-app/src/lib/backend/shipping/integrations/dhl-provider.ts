...
import { ShippingStatus } from '@backend/shipping/shipping.interface';
import {
  ShippingProviderAPI,
  ShipmentAddress,
  ShipmentDetails,
...
import { shippingConfig } from '@backend/config';
class DHLProvider implements ShippingProviderAPI {
  ...
  async createShipment(details: ShipmentDetails): Promise<ShipmentResponse> {
    ...
    return {
      ...
      labelUrl: response.data.documents.find((doc: any) => doc.type === 'label').url,
  ...
  async getServiceAreas(): Promise<{ country: string; postalCodes: string[] }[]> {
    ...
    return response.data.serviceAreas.map((area: any) => ({
