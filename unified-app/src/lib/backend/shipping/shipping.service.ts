...
import { ShippingProvider, ShippingZone, ShippingRate, ShippingLabel, ShippingPackage, ShippingStatus } from '@backend/shipping/shipping.interface';
import {
...
} from '@backend/shipping/shipping.model';
import {
  CreateProviderDto,
  UpdateProviderDto,
  CreateZoneDto,
...
class ShippingService {
  ...
  public async addTrackingHistory(labelId: string, historyData: AddTrackingHistoryDto): Promise<ShippingLabel> {
    ...
    (label.trackingHistory ?? (label.trackingHistory = [])).push(trackingEntry);
